...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const { informacionUsuario, programa, asignaturas, credenciales } = datosEstudiante;
...
function renderizarInformacionEstudiante(info) {
    ...
    // ... (El resto de la función es exactamente igual a la respuesta anterior) ...
    if (!info || Object.keys(info).length === 0) {
        tabla.innerHTML = '<tr><td colspan="2">No hay información de estudiante.</td></tr>';
        return;
    }
    ...
    let html = `
        ...
        <tr><th>Tipo de Identificación</th><td>${info.tipo_identificacion}</td></tr>
        <tr><th>Número de Identificación</th><td>${info.numero_identificacion}</td></tr>
        <tr><th>Estado / Cond. Matrícula</th><td>${info.estado} / ${info.cond_matricula}</td></tr>
        ...
        <tr>
            ...
            <td>
                ${info.nivel}
                ${info.nivel !== 'Nivel 1' ? '<br><a href="https://site2.q10.com/Prematricula" target="_blank">¡Prematrículate aquí!</a> / <a href="..." target="_blank">¿Cómo me prematriculo?</a>' : ''}
            </td>
        </tr>
...
function renderizarAsignaturas(asignaturas, programa) {
    ...
    if (!asignaturas || asignaturas.length === 0) {
        tabla.innerHTML = '<tr><td colspan="2">No hay programas asignados.</td></tr>';
        return;
    }
    ...
    for (const nivel in asignaturasPorNivel) {
        if (asignaturasPorNivel[nivel].length > 0) {
            const titulo = nivel === 'otros' ? 'Prácticas y Vinculación / Ingles:' : `Nivel ${nivel.replace('n', '')}:`;
            asignaturasHtml += `<ul><strong>${titulo}</strong>`;
            asignaturasHtml += asignaturasPorNivel[nivel].map(asig => `<li>${asig}</li>`).join('');
            asignaturasHtml += `</ul>`;
        }
    }
...
function renderizarCredenciales(credenciales, usuario) {
    ...
    if (!credenciales || credenciales.length === 0) {
        tabla.innerHTML = '<tr><td colspan="3">No hay credenciales disponibles.</td></tr>';
        return;
    }
    ...
    const urls = {
        ...
        'eLibro': 'https://elibro.net/es/lc/superarse/login_usuario/?next=/es/lc/superarse/inicio/'
    };
    ...
    const html = credenciales.map(credencial => {
        ...
        return `
            <tr>
                ...
                <td>${credencial.password_hash}</td>
            </tr>
        `;
    }).join('');
